                    ...
                    <tbody>
                        @foreach ($sanPhams as $sanPham)
                            ...
                            <tr id="product-row-{{ $sanPham->ma_san_pham }}">
                                <td><img src="{{ $sanPham->duong_dan_hinh_anh }}" alt="{{ $sanPham->ten_hien_thi }}" class="image-product"></td>
                                        ...
                                        <div class="modal-body">
                                            <form id="update-product-{{ $sanPham->ma_san_pham }}" action="{{ route('admin.san-pham.cap-nhat') }}" method="POST" class="form-horizontal form-label-left" enctype="multipart/form-data">
                                                ...
                                                <div class="item form-group">
                                                    ...
                                                    <div class="col-md-8">
                                                        <label class="custom-file-upload" for="product-images-{{ $sanPham->ma_san_pham }}">Chọn ảnh</label>
                                                        <input type="file" name="images[]" class="product-images" id="product-images-{{ $sanPham->ma_san_pham }}" data-id="{{ $sanPham->ma_san_pham }}" accept="image/*" multiple>
                                                        <div id="image-preview-container-{{ $sanPham->ma_san_pham }}" class="image-preview-container image-preview-listproduct" data-id="{{ $sanPham->ma_san_pham }}">
                                                            @foreach ($sanPham->hinhAnhs as $hinhAnh)
                                                                <img src="{{ asset('storage/'.$hinhAnh->hinh_anh) }}" alt="Ảnh sản phẩm" class="image-preview">
                                                            @endforeach
                                                        </div>
                                                    </div>
                                                </div>
